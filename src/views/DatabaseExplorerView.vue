...
const lastViewedConnectionId = ref<string>(
    localStorage.getItem('lastViewedConnectionId') || ''
)
...
// Selected tab index
const selectedIndex = computed(() => recentConnections.value.findIndex(c => c.id === currentConnectionId.value))
...
function initializeCurrentConnection() {
    if (currentConnection.value && !recentConnections.value.find(c => c.id === currentConnectionId.value)) {
...
watch(
    ...
    (newPath) => {
        if (newPath === '/explorer' && recentConnections.value.length > 0) {
            ...
            const connectionToUse = lastViewedConnectionId.value &&
