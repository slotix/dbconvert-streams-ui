    ...
    <div v-if="apiKey" class="mb-4">
      ...
      <div class="mt-1 relative rounded-md shadow-sm">
        ...
        <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
          <button class="text-gray-400 hover:text-gray-500" @click="toggleApiKeyVisibility">
            <component :is="showApiKey ? EyeSlashIcon : EyeIcon" class="h-5 w-5" />
          </button>
          <button class="ml-2 text-gray-400 hover:text-gray-500" @click="copyApiKey">
  ...
  if (confirm('Are you sure you want to update your API key? This will generate a new key.')) {
    try {
      ...
      commonStore.showNotification(
        'Your API key has been updated. Please make sure to use the new key in your requests.',
        'success'
      )
    } catch (error) {
      console.error('Error updating API key:', error)
      commonStore.showNotification(
        'An error occurred while updating your API key. Please try again.',
        'error'
      )
    }
  }
