<template>
  <template v-if="showFallback">
    <section
      v-if="isPanelOpen"
      class="fixed right-5 bottom-24 z-[121] h-[560px] w-[360px] overflow-hidden rounded-xl border border-[#24496b]/60 bg-gray-950 shadow-2xl"
    >
      <header
        class="flex items-center justify-between border-b border-gray-800 px-3 py-2"
        style="background-color: #24496b"
      >
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            class="grid h-7 w-7 place-items-center rounded-md text-white transition-colors hover:bg-[#2d5a80]"
    ...
    <button
      ...
      class="fixed right-5 bottom-5 z-[122] grid h-16 w-16 place-items-center rounded-full bg-transparent text-white shadow-lg transition-all hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0a2a45]/80 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-950"
      @click="togglePanel"
    >
      <svg
        ...
        aria-hidden="true"
      >
        <circle cx="32" cy="32" r="30" fill="#0a2a45" stroke="#0a2a45" stroke-width="2" />
        <path
