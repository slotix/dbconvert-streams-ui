      ...
      <div v-else class="flex-grow space-y-2 px-4 py-3 bg-white">
        <div class="relative">
          <div class="grid grid-cols-2 gap-3">
            <div
              class="flex-auto space-y-1.5 text-gray-500 bg-gray-50/50 rounded-lg p-2.5 border border-gray-100"
            >
              <span class="mx-auto font-semibold text-gray-900">
                <div class="flex items-center justify-center space-x-2 mb-2">
                  ...
                  <img
                    ...
                    class="h-5 w-5 rounded-full ml-1.5"
                    :src="logoSrc(source.type)"
                    :alt="source.type + ' logo'"
                  />
                </div>
                <span
                  class="font-normal text-sm break-all block mt-1.5 flex items-center gap-1"
                  :class="{
                    'text-red-500': !source || !source.name,
                    'text-gray-600': source && source.name
                  }"
                >
                  {{ source?.name || 'N/A' }}
            ...
            <div
              class="absolute left-1/2 top-1/2 -translate-y-1/2 -ml-px border-l-2 border-gray-300 h-4/5"
            ></div>
              ...
              <span class="mx-auto font-semibold text-gray-900">
                ...
                <span
                  ...
                  :class="{
                    ...
                    'text-gray-600': target && target.name
                  }"
                >
                  {{ target?.name || 'N/A' }}
                  <ExclamationCircleIcon
