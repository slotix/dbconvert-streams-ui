<template>
  <div class="stream-config-editor">
    <!-- Header with buttons -->
...
watch(
  () => props.config,
  (newConfig, oldConfig) => {
    // Only reinitialize if the config ID changed (switched to different stream)
    // or if we're not dirty (no unsaved local changes)
    if (newConfig.id !== oldConfig?.id || !isDirty.value) {
      initializeContent()
    }
  },
  { deep: true }
...
const debouncedValidate = debounce(() => {
  const result = parseAndValidateStreamConfig(editorContent.value, props.config)
...
const handleSave = async () => {
  ...
  try {
    const updatedConfig = result.parsed as StreamConfig
