...
import { CheckIcon, XCircleIcon, PauseCircleIcon } from '@heroicons/vue/24/solid'
...
const normalizedStatus = computed(() => props.status?.toUpperCase?.() ?? props.status)

const statusClass = computed(() => {
  switch (normalizedStatus.value) {
    case 'FINISHED':
      return 'bg-green-200 dark:bg-green-900/40 text-green-900 dark:text-green-300'
    case 'RUNNING':
