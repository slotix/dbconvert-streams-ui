...
function formatColumnDef(col: Column): string {
  ...
  const comment = [col.isPrimaryKey && 'PK', col.isForeignKey && 'FK'].filter(Boolean).join(', ')
  ...
  if (props.relationships?.length) {
    ...
    props.relationships.forEach((rel) => {
      ...
      if (isJunctionTable(sourceTable.columns)) {
        ...
        const otherRel = props.relationships.find(
          (r) => r.sourceTable === rel.sourceTable && r.id !== rel.id
        )
        if (otherRel) {
...
watch(
  [() => props.tables, () => props.views, () => props.relationships],
  () => {
    debouncedRender()
  },
  { deep: true }
...
<template>
  <div class="mermaid-container">
    <div class="controls">
      ...
      <button
        @click="transform.scale = Math.max(0.5, transform.scale * 0.9)"
        class="control-btn"
        title="Zoom out"
      >
        <svg
    ...
    <div v-if="isLoading" class="loading">Loading diagram...</div>
