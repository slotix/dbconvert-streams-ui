...
<template>
    <div class="p-4">
        <div class="flex items-center justify-between mb-3">
            ...
            <button type="button"
                class="px-2 py-1 text-xs rounded border border-gray-300 bg-white hover:bg-gray-50 text-gray-700" @click="
                    emit('show-diagram', {
        ...
        <div v-else-if="summary" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div v-if="summary.schemas && summary.stats.schemas" class="bg-white ring-1 ring-gray-900/5 rounded-lg p-4">
                <div class="text-sm text-gray-500">Schemas</div>
                <div class="text-2xl font-semibold">{{ summary.stats.schemas }}</div>
                <ul class="mt-2 text-sm text-gray-700 space-y-1">
                    <li v-for="s in (summary.schemas || []).slice(0, 10)" :key="s" class="truncate" :title="s">
                        {{ s }}
                    </li>
                </ul>
            </div>
