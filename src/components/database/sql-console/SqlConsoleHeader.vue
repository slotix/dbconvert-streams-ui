<template>
  ...
  >
    <div class="flex items-center gap-3">
      <Terminal class="h-5 w-5 text-teal-500" />
      <div>
    ...
    <div class="flex items-center space-x-2">
      ...
      <div class="relative">
        <button
          ...
          :class="{ 'opacity-50 cursor-not-allowed': historyCount === 0 }"
          @click="$emit('toggle-history')"
        >
          <Clock class="h-3.5 w-3.5 mr-1" />
          History
          <span v-if="historyCount > 0" class="ml-1 text-gray-400">({{ historyCount }})</span>
        </button>
      </div>
    </div>
  </div>
...
import { ChevronDown, Clock, FileText, Terminal } from 'lucide-vue-next'
