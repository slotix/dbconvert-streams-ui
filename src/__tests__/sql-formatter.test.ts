...
describe('SQL Formatter', () => {
  it('should format a simple SQL query', () => {
    const unformattedSQL =
      'SELECT id,name,email FROM users WHERE status=1 AND age>18 ORDER BY name ASC'
  ...
  it('should format a complex SQL query with joins', () => {
    const unformattedSQL =
      'SELECT u.id,u.name,o.order_date,p.product_name FROM users u LEFT JOIN orders o ON u.id=o.user_id INNER JOIN products p ON o.product_id=p.id WHERE o.status="completed" GROUP BY u.id HAVING COUNT(o.id)>5'
  ...
  it('should format INSERT statement', () => {
    const unformattedSQL =
      'INSERT INTO users(id,name,email,created_at)VALUES(1,"John Doe","john@example.com",NOW())'
  ...
  it('should format CREATE TABLE statement', () => {
    const unformattedSQL =
      'CREATE TABLE `country` (`country_id` smallint unsigned NOT NULL AUTO_INCREMENT,`country` varchar(50) NOT NULL,`last_update` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,PRIMARY KEY (`country_id`)) ENGINE=InnoDB AUTO_INCREMENT=110 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci'
  ...
  it('should format CREATE INDEX statements', () => {
    const unformattedSQL =
      'CREATE INDEX `idx_title` (`title`) ON `film`;CREATE INDEX `idx_fk_original_language_id` (`original_language_id`) ON `film`'
